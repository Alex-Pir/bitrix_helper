 ...
 * а в дополнительном как 2. Это позволит нам при сортировке сначала использовать "GROUP", что уже не даст смешиваться результатам. В качестве второй сортировки возьмем "NAME". Первая группа тоже отсортируется по "NAME", но они хотя бы будут отдельно друг от друга.
 */
$result = $class::query()
    ->addSelect("ID")
    ->addSelect("NAME")
